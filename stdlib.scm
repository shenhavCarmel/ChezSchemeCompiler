...
(define fold-left
	(let ((null? null?)
		(car car) (cdr cdr))
		(letrec ((fold-left-loop (lambda (f start lst) (if (null? lst) start (fold-left-loop f (f start (car lst)) (cdr lst)))))) fold-left-loop)))


(define fold-right
	(let ((null? null?)
		(car car) (cdr cdr))
  		(letrec ((fold-right-loop (lambda (f start lst) (if (null? lst) start (f (car lst) (fold-right-loop f start (cdr lst))))))) fold-right-loop)))

(define cons*
	(let ((null? null?)
		(cdr cdr) (car car))
  		(letrec ((cons-loop (lambda args (if (null? (cdr args)) (car args) (cons (car args) (apply cons-loop (cdr args))))))) cons-loop)))
